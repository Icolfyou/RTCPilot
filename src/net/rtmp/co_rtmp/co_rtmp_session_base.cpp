...
CoRtmpSessionBase::CoRtmpSessionBase(Logger* logger):recv_buffer_(50*1024)
                                                 , logger_(logger)
{
}

CoRtmpSessionBase::~CoRtmpSessionBase()
{
}

int CoRtmpSessionBase::ReadFmtCsid() {
    uint8_t* p = nullptr;
...
void CoRtmpSessionBase::SetChunkSize(uint32_t chunk_size) {
    chunk_size_ = chunk_size;
}

uint32_t CoRtmpSessionBase::GetChunkSize() {
    return chunk_size_;
}

bool CoRtmpSessionBase::IsPublish() {
    return req_.publish_flag_;
}

const char* CoRtmpSessionBase::IsPublishDesc() {
    return req_.publish_flag_ ? "publish" : "play";
}


Media_Packet_Ptr CoRtmpSessionBase::GetMediaPacket(CHUNK_STREAM_PTR cs_ptr) {
...
int CoRtmpSessionBase::ReadChunkStream(CHUNK_STREAM_PTR& cs_ptr) {
    int ret = -1;

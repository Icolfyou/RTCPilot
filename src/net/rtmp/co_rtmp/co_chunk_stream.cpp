...
CoChunkStream::CoChunkStream(CoRtmpSessionBase* session, 
        uint8_t fmt, 
        uint16_t csid, 
        uint32_t chunk_size,
...
CoChunkStream::~CoChunkStream() {

}

bool CoChunkStream::IsReady() {
    return chunk_ready_;
}

int CoChunkStream::GenSetRecordedMessage() {
    uint8_t data[6];
    ...
    data[0] = (((uint16_t)Event_streamIsRecorded) >> 8) & 0xff;
...
int CoChunkStream::GenSetBeginMessage() {
    uint8_t data[6];
    ...
    data[0] = (((uint16_t)Event_streamBegin) >> 8) & 0xff;
...
int CoChunkStream::GenControlMessage(RTMP_CONTROL_TYPE ctrl_type, uint32_t size, uint32_t value) {
    fmt_           = 0;
...
int CoChunkStream::ReadMsgFormat0(uint8_t input_fmt, uint16_t input_csid) {
...
int CoChunkStream::ReadMsgFormat1(uint8_t input_fmt, uint16_t input_csid) {
    DataBuffer* buffer_p = session_->GetRecvBuffer();
...
int CoChunkStream::ReadMsgFormat2(uint8_t input_fmt, uint16_t input_csid) {
    DataBuffer* buffer_p = session_->GetRecvBuffer();
...
int CoChunkStream::ReadMsgFormat3(uint8_t input_fmt, uint16_t input_csid) {
    DataBuffer* buffer_p = session_->GetRecvBuffer();
    uint8_t* p;
...
int CoChunkStream::ReadMessageHeader(uint8_t input_fmt, uint16_t input_csid) {
    int ret = -1;
...
int CoChunkStream::ReadMessagePayload() {
    ...
    buffer_p->ConsumeData((int)require_len_);
...
void CoChunkStream::DumpPayload() {
    char desc[128];

    snprintf(desc, sizeof(desc), "chunk stream payload:%zu", chunk_data_ptr_->DataLen());
    LogInfoData(logger_, (uint8_t*)chunk_data_ptr_->Data(), chunk_data_ptr_->DataLen(), desc);
}

void CoChunkStream::DumpAllData() {
    char desc[128];

    snprintf(desc, sizeof(desc), "chunk stream all data:%zu", chunk_all_ptr_->DataLen());
    LogInfoData(logger_, (uint8_t*)chunk_all_ptr_->Data(), chunk_all_ptr_->DataLen(), desc);
}

int CoChunkStream::GenData(uint8_t* data, int len) {
    if (csid_ < 64) {
...
int WriteDataByChunkStream(CoRtmpSessionBase* session, uint16_t csid,
                    uint32_t timestamp, uint8_t type_id,
                    uint32_t msg_stream_id, uint32_t chunk_size,
                    DataBuffer& input_buffer,
                    Logger* logger)
{
    int cs_count = (int)input_buffer.DataLen()/(int)chunk_size;
    int fmt = 0;
    uint8_t* p;
    int len;
    ...
    for (int index = 0; index < cs_count; index++) {
        ...
        CoChunkStream* c = new CoChunkStream(session, fmt, csid, chunk_size, logger);

        c->timestamp32_ = timestamp;
        c->msg_len_     = (uint32_t)input_buffer.DataLen();
        c->type_id_     = type_id;
...
int WriteDataByChunkStream(CoRtmpSessionBase* session, uint16_t csid,
                    ...
                    Logger* logger)
{
    int cs_count = (int)input_buffer_ptr->DataLen()/(int)chunk_size;
    int fmt = 0;
    uint8_t* p;
    int len;
    ...
    for (int index = 0; index < cs_count; index++) {
        ...
        c->timestamp32_ = timestamp;
        c->msg_len_     = (uint32_t)input_buffer_ptr->DataLen();
        c->type_id_     = type_id;
        c->msg_stream_id_ = msg_stream_id;
        c->GenData(p, len);
        ...
        //c->DumpHeader();
        //c->DumpAllData();
        int ret = session->RtmpSend(c->chunk_all_ptr_);
        if (ret < 0) {
            delete c;
            return ret;
        }

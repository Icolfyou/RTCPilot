...
CoRtmpControlHandler::CoRtmpControlHandler(CoRtmpSessionBase* session,
        Logger* logger):session_(session)
                        , logger_(logger)
{
    LogInfof(logger, "CoRtmpControlHandler construct...");
}

CoRtmpControlHandler::~CoRtmpControlHandler()
{
}

int CoRtmpControlHandler::HandleServerCommandMessage(CHUNK_STREAM_PTR cs_ptr, std::vector<AMF_ITERM*>& amf_vec) {
...
int CoRtmpControlHandler::HandleClientCommandMessage(CHUNK_STREAM_PTR cs_ptr, std::vector<AMF_ITERM*>& amf_vec) {
    int ret = 0;
    uint8_t* data = (uint8_t*)cs_ptr->chunk_data_ptr_->Data();
    int len = (int)cs_ptr->chunk_data_ptr_->DataLen();
	...
	LogInfof(logger_, "rtmp client command type:%s", cmd_type.c_str());
...
int CoRtmpControlHandler::HandleRtmpConnectCommand(uint32_t stream_id, std::vector<AMF_ITERM*>& amf_vec) {
    if (amf_vec.size() < 3) {
...
int CoRtmpControlHandler::HandleRtmpCreatestreamCommand(uint32_t stream_id, std::vector<AMF_ITERM*>& amf_vec) {
    if (amf_vec.size() < 3) {
...
int CoRtmpControlHandler::HandleRtmpPlayCommand(uint32_t stream_id, std::vector<AMF_ITERM*>& amf_vec) {
    if (amf_vec.size() < 3) {
...
int CoRtmpControlHandler::HandleRtmpPublishCommand(uint32_t stream_id, std::vector<AMF_ITERM*>& amf_vec) {
    if (amf_vec.size() < 3) {
...
int CoRtmpControlHandler::SendSetChunksize(uint32_t chunk_size) {
    CoChunkStream set_chunk_size_cs(session_, 0, 3, session_->GetChunkSize(), logger_);
...
int CoRtmpControlHandler::SendRtmpConnectResp(uint32_t stream_id) {
    DataBuffer amf_buffer;
    CoChunkStream win_size_cs(session_, 0, 3, session_->GetChunkSize(), logger_);
    CoChunkStream peer_bw_cs(session_, 0, 3, session_->GetChunkSize(), logger_);
    CoChunkStream set_chunk_size_cs(session_, 0, 3, session_->GetChunkSize(), logger_);
...
int CoRtmpControlHandler::SendRtmpCreateStreamResp(double transaction_id) {
    DataBuffer amf_buffer;
    std::string result_str = "_result";
...
int CoRtmpControlHandler::SendRtmpPlayResp() {
    uint32_t chunk_size = 6;

    //send rtmp control recorded
    CoChunkStream ctrl_recorded(session_, 0, 3, chunk_size, logger_);
    ctrl_recorded.GenSetRecordedMessage();
    session_->RtmpSend(ctrl_recorded.chunk_all_ptr_);

    //send rtmp control begin
    CoChunkStream ctrl_begin(session_, 0, 3, chunk_size, logger_);
...
int CoRtmpControlHandler::SendRtmpPlayResetResp() {
    DataBuffer amf_buffer;
...
int CoRtmpControlHandler::SendRtmpPlayStartResp() {
    DataBuffer amf_buffer;
    double transaction_id = 0.0;
...
int CoRtmpControlHandler::SendRtmpPlayDataResp() {
    DataBuffer amf_buffer;
    double transaction_id = 0.0;
...
int CoRtmpControlHandler::SendRtmpPlayNotifyResp() {
    DataBuffer amf_buffer;
    double transaction_id = 0.0;
...
int CoRtmpControlHandler::SendRtmpPublishResp() {
    DataBuffer amf_buffer;
    double transaction_id = 0.0;
...
int CoRtmpControlHandler::SendRtmpAck(uint32_t size) {
    int ret = 0;
    session_->ack_received_ += size;

    if (session_->ack_received_ > session_->remote_window_acksize_) {
        CoChunkStream ack(session_, 0, 3, session_->GetChunkSize(), logger_);
...
int CoRtmpControlHandler::HandleRtmpControlMessage(CHUNK_STREAM_PTR cs_ptr, bool is_server) {
    if (cs_ptr->type_id_ == RTMP_CONTROL_SET_CHUNK_SIZE) {
        ...
        LogDebugf(logger_, "update chunk size:%u, is_server:%s",
